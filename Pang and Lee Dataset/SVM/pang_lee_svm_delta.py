...
def grab_pos_data(path,y_pos):
    ...
    for ff in glob.glob("*.txt"):
        with open(ff, 'r') as f:
            pos_sentences.append(f.read().strip())
            y_pos.append(1)
...
def grab_neg_data(path, y_neg):
    ...
    for ff in glob.glob("*.txt"):
        with open(ff, 'r') as f:
            neg_sentences.append(f.read().strip())
            y_neg.append(0)
    os.chdir(currdir)
...
pos_sentences = grab_pos_data(path, y_pos)
neg_sentences = grab_neg_data(path, y_neg)
sentences = grab_data(path,y)

def DeltaTfidfVectorizer(pos_sentences, neg_sentences,y):

    np.zeros((10000,len(y)))
